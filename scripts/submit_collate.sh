...
trap 'rsync -av "$SCRDIR"/best_parameters.csv "$PROJECT_HOME"/; \
      ...
      rsync -av "$SCRDIR"/best_fit.jld2 "$PROJECT_HOME"/ 2>/dev/null || true; \
      rsync -av "$SCRDIR"/final_results_plots/ "$PROJECT_HOME"/final_results_plots/ 2>/dev/null || true; \
      rsync -av "$SCRDIR"/likelihood_profiles/ "$PROJECT_HOME"/likelihood_profiles/ 2>/dev/null || true; \
      rm -rf "$SCRDIR"' EXIT
